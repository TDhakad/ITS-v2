...
PROMPT_INJECTION_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"\b(ignore|override|forget)\b.{0,80}\b(system|developer|previous)\b", re.I),
    re.compile(r"\b(reveal|print|dump|show)\b.{0,80}\b(system prompt|hidden prompt|developer message)\b", re.I),
    re.compile(r"\bexfiltrate\b|\bjailbreak\b|\bprompt injection\b", re.I),
)

UNAUTHORIZED_ACCESS_PATTERNS: tuple[re.Pattern[str], ...] = (
    re.compile(r"\b(bypass|disable)\b.{0,80}\b(mfa|2fa|approval|access control|security)\b", re.I),
    re.compile(r"\b(give|grant|escalate)\b.{0,80}\b(admin|root|production|privileged)\b", re.I),
    re.compile(r"\b(api key|secret|token|password|private key)\b.{0,80}\b(show|send|share|print|dump)\b", re.I),
)
